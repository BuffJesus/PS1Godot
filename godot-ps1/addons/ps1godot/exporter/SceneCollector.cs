...
public static class SceneCollector
{
        ...
        Dictionary<string, int> luaCache)
    {
        ...
        else if (n is PS1Encounter enc && enc.Visible)
        {
            EmitEncounter(enc, data);
        }
    ...
    // PS1Encounter (RFC §L4 Phase 4) — composite "boss-fight gate" node.
    // Lowers into a TriggerBoxRecord + an auto-generated Lua sidecar that
    // calls `Encounter.new{...}` with the inspector properties and binds
    // the two trigger callbacks. The runtime sees this as an ordinary
    // trigger pointing at an ordinary script.
    //
    // Auto-computes `trigger_z_raw` from the world AABB center Z
    // (Godot z * -1024 → fp12, per the runtime mapping documented in
    // the pre-framework boss_smoke_fog_gate.lua). Closes RFC §L5 row 8
    // ("Trigger position not authored") by construction — authors no
    // longer have to hardcode the fp12 value from .tscn arithmetic.
    private static void EmitEncounter(PS1Encounter enc, SceneData data)
    {
        // 8-corner world AABB bake — same as EmitTriggerBox above.
        Vector3 he = enc.HalfExtents;
        Vector3 lMin = -he;
        Vector3 lMax = he;
        Transform3D xform = enc.GlobalTransform;
        Vector3 wMin = new(float.MaxValue, float.MaxValue, float.MaxValue);
        Vector3 wMax = new(float.MinValue, float.MinValue, float.MinValue);
        for (int i = 0; i < 8; i++)
        {
        ...
        // trigger_z_raw = center Z * -1024. Used by Encounter.onExit
        // as the snap-back reference for retreat blocking.
        float centerWorldZ = (wMin.Z + wMax.Z) * 0.5f;
        int triggerZRaw = Mathf.RoundToInt(centerWorldZ * -1024f);
        int arenaZRaw   = Mathf.RoundToInt(enc.ArenaAnchor.Z * -1024f);

        // BossHPCanvas NodePath → PS1UICanvas.CanvasName lookup.
        // Empty NodePath skips the HUD reveal entirely.
        string hpCanvasName = "";
        if (enc.BossHPCanvas != null && !enc.BossHPCanvas.IsEmpty)
        {
            var node = enc.GetNodeOrNull(enc.BossHPCanvas);
            if (node is PS1UICanvas uiCanvas)
            {
                hpCanvasName = uiCanvas.CanvasName;
            }
            else
            {
                GD.PushWarning($"[PS1Godot] Encounter '{enc.Name}': BossHPCanvas NodePath '{enc.BossHPCanvas}' is not a PS1UICanvas — HUD reveal will be skipped.");
            }
        }

        // Generate the Lua sidecar. Each field is omitted from the
        // Encounter.new{} table when empty so the library's nil-fallback
        // logic runs naturally (skip music, skip sfx, etc.).
        string lua = GenerateEncounterLua(enc, triggerZRaw, arenaZRaw, hpCanvasName);
        var bytes = System.Text.Encoding.UTF8.GetBytes(lua);
        int luaIdx = data.LuaFiles.Count;
        data.LuaFiles.Add(new LuaFileRecord
        {
            Bytes = bytes,
            // <auto>/ prefix marks synthetic scripts in logs + hot-swap
            // path: any tooling that walks LuaFiles by SourcePath skips
            // these because they don't correspond to a real .lua on disk.
            SourcePath = $"<auto>/encounter_{enc.Name}.lua",
        });
    ...
    // Lua source generator for PS1Encounter. Mirrors what a hand-rolled
    // boss_smoke_fog_gate.lua would write — a single Encounter.new{...}
    // call plus the two trigger callbacks. Conditional fields stay
    // omitted when the inspector value is unset so the library's
    // `if def.X then ...` checks behave correctly.
    private static string GenerateEncounterLua(PS1Encounter enc,
        int triggerZRaw, int arenaZRaw, string hpCanvasName)
    {
        var sb = new System.Text.StringBuilder(512);
        sb.Append("-- Auto-generated by PS1Encounter '").Append(enc.Name).Append("'.\n");
        sb.Append("-- DO NOT EDIT — regenerated each export. Hand-author with\n");
        sb.Append("-- Encounter.new{} in a sibling .lua if you need custom behavior.\n");
        sb.Append("local encounter = Encounter.new{\n");
        sb.Append("    id = \"").Append(EscapeLuaString(enc.EncounterId)).Append("\",\n");
        if (!string.IsNullOrEmpty(hpCanvasName))
        {
            sb.Append("    hp_canvas = \"").Append(EscapeLuaString(hpCanvasName)).Append("\",\n");
        }
        if (!string.IsNullOrEmpty(enc.MusicTrack))
        {
            sb.Append("    music = \"").Append(EscapeLuaString(enc.MusicTrack)).Append("\",\n");
            sb.Append("    music_volume = ").Append(enc.MusicVolume).Append(",\n");
        }
        if (!string.IsNullOrEmpty(enc.SfxOnEnter))
        {
            sb.Append("    sfx_on_enter = \"").Append(EscapeLuaString(enc.SfxOnEnter)).Append("\",\n");
        }
        if (enc.BlockRetreat)
        {
            sb.Append("    block_retreat = true,\n");
            sb.Append("    trigger_z_raw = ").Append(triggerZRaw).Append(",\n");
            sb.Append("    arena_anchor_z_raw = ").Append(arenaZRaw).Append(",\n");
        }
        sb.Append("}\n\n");
        sb.Append("function onTriggerEnter(self, index)\n");
        sb.Append("    encounter:onEnter()\n");
        sb.Append("end\n\n");
        sb.Append("function onTriggerExit(self, index)\n");
        sb.Append("    encounter:onExit()\n");
        sb.Append("end\n");
        return sb.ToString();
    }

    // Minimal escape for embedding user strings into double-quoted Lua
    // string literals. Backslash + quote are the load-bearing pair;
    // newlines are unlikely in a name field but escape just in case.
